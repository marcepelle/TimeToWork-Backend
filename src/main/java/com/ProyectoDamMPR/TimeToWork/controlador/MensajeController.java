...
public class MensajeController {
    ...
    @GetMapping("/obtenerRecibidos") //A través de un método Get en la ruta declarada se devuelve el listado de mensajes recibidos para el correo pasado en la petición
    public ArrayList<Mensaje> getRecibidos(@RequestParam("correo") String correo){ //Con RequestParam obtenemos el/los parámetro/s pasado/s en la petición
        System.out.println("Obteniendo Recibidos:......");
        return mensajeDao.getMensajesRecibidos(correo);
    }

    @GetMapping("/obtenerEnviados") //A través de un método Get en la ruta declarada se devuelve un listado de mensajes envíados para el correo pasado en la petición
    public ArrayList<Mensaje> getEnviados(@RequestParam("correo") String correo){ //Con RequestParam obtenemos el/los parámetro/s pasado/s en la petición
        System.out.println("Obteniendo Enviados:......");
        return mensajeDao.getMensajesEnviados(correo);
    }

    @PutMapping("/mensajeVistoDe")  //A través de un método Put en la ruta declarada se fijará como true el atributo vistoDe del mensaje que se envío en la petición
    ...
    @PutMapping("/mensajeVistoPara") //A través de un método Put en la ruta declarada se fijará como true el atributo vistoPara del mensaje que se envío en la petición
