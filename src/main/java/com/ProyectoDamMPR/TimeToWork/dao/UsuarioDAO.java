...
public interface UsuarioDAO { //Con esta interfaz declaramos los métodos abstractos de las funciones CRUD(Crear, leer, actualizar y borrar) para la entidad Usuario
    ...
    public Usuario getUsuario(String correo); //Devuelve el usuario buscado a través del correo pasado
    ...
    public Usuario updateContrasena(Usuario usuario); //Actualiza la contraseña y devuelve el usuario actualizado que se ha pasado
    public int RemoveUsuario(int idUsuario); //Devuelve una respuesta de tipo entero dependiendo de si se consiguió eliminar o no el usuario para el id pasado
